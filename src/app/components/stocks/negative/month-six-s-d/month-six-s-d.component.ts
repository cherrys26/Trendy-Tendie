...
@Component({
  selector: 'app-month-six-s-d',
  templateUrl: './month-six-s-d.component.html',
  styleUrls: ['./month-six-s-d.component.scss'],
})
export class MonthSixSDComponent implements OnInit {
  ...
  stockData() {
    ...
    this.stockService.getStock('pm').subscribe(stockThree => { this.three = stockThree });
  ...
  stockPrice() {
    ...
    this.stockService.getPrice('pm').subscribe(stockThree => { this.threeP = stockThree });
